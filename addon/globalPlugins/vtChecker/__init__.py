...
class GlobalPlugin(globalPluginHandler.GlobalPlugin):
	...
	def do_check(self, full_report=False):
		...
		def run():
			try:
				...
				if sha256 in self.polling_hashes:
					core.callLater(10, ui.message, _("File is already being analyzed. Please wait..."))
					return
				...
				if res is None: # Not found
					if config.conf["vtChecker"].get("autoUpload", False):
						...
						core.callLater(10, ui.message, _("File uploaded and queued for analysis. Please wait, I will notify you when it's done."))
						
						self.polling_hashes.add(sha256)
						
						def poll_hash():
							import time
							try:
								for loop_idx in range(20): # 5 minutes max
									time.sleep(15)
									try:
										poll_res = vt_api.check_hash(sha256, api_key)
										if poll_res:
											stats = poll_res.get("data", {}).get("attributes", {}).get("last_analysis_stats", {})
											total = sum(stats.values()) if stats else 0
											if total > 0:
												# Finished
												summary = vt_api.format_short_summary(poll_res)
												report = vt_api.format_detailed_report(poll_res)
												...
												core.callLater(10, ui.message, _("Analysis complete: ") + summary)
												if config.conf["vtChecker"].get("autoCopy", True):
													nvda_api.copyToClip(report_with_link)
													core.callLater(3000, ui.message, _("Result copied to clipboard."))
												return
									except Exception:
										pass
								core.callLater(10, ui.message, _("Analysis is taking too long. Please press the shortcut later."))
							finally:
								if sha256 in self.polling_hashes:
									self.polling_hashes.remove(sha256)
							
						t_poll = threading.Thread(target=poll_hash)
						t_poll.daemon = True
						t_poll.start()
						return
					else:
	...
	def do_manual_check(self, input_val):
		...
		def run():
			try:
				import re
				# Extract hash if user pasted a VT link by mistake
				vt_match = re.search(r"virustotal\.com/gui/file/([a-fA-F0-9]{64})", input_val)
				val_to_check = vt_match.group(1) if vt_match else input_val
				
				# Determine if hash or URL
				if re.match(r"^[a-fA-F0-9]{32}$|^[a-fA-F0-9]{40}$|^[a-fA-F0-9]{64}$", val_to_check):
					is_url = False
				elif "://" in val_to_check or "." in val_to_check:
					is_url = True
				else:
					is_url = False
				...
				if is_url:
					res = vt_api.check_url(val_to_check, api_key)
					import base64
					url_id = base64.urlsafe_b64encode(val_to_check.encode('utf-8')).decode('utf-8').strip("=")
					vt_url = f"https://www.virustotal.com/gui/url/{url_id}"
				else:
					res = vt_api.check_hash(val_to_check, api_key)
					vt_url = f"https://www.virustotal.com/gui/file/{val_to_check}"
